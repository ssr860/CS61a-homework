...
class BankAccount:
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        if amount > self.balance:
            self.transactions.append(Transaction(self.id,before,before))
            self.id+=1
...
class Coin:
    ...
    def worth(self) -> int:
        "*** YOUR CODE HERE ***"
        extra_cent=Mint.present_year-self.year
        if extra_cent>50:
            return self.cents+extra_cent-50
        else:
            return self.cents
...
class VirFib():
    ...
    # def __init__(self, value: int = 0,pre=1):
    #     self.value = value
    #     self.pre=pre
    ...
    #     "*** YOUR CODE HERE ***"
    #     self.pre=self.value
    #     self.value=self.pre+self.value
    #     return VirFib(self.value,self.pre) 
