...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x=(get_lat(city_a)-get_lat(city_b))**2+(get_lon(city_b)-get_lon(city_a))**2
    return sqrt(x)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x=sqrt((lat-get_lat(city_a))**2+(lon-get_lon(city_a))**2)
    y=sqrt((lat-get_lat(city_b))**2+(lon-get_lon(city_b))**2)
    if y<=x:
