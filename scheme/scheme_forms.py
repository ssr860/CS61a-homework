...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        m=scheme_eval(expressions.first,env)
        if is_scheme_false(m) and expressions.rest is not nil:
            return do_or_form(expressions.rest,env)
        elif is_scheme_false(m) and expressions.rest is nil:
            return False
        else:
            return m
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        validate_form(bindings.first,2,2)#bingding只能由一个symbol和一个具体值构成为2
        symbol=bindings.first.first
        expr=bindings.first.rest.first
...
def check_formals(formals):
    ...
    while formals is not nil:
        ...
        if not scheme_symbolp(sym):
            raise SchemeError('invalid formals')
        if sym in seen:
            raise SchemeError('duplicate formal')
        seen.add(sym)
