...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            place.ant=self
        elif place.ant.is_container and not self.is_container and place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        while place and not place.is_hive:
            if place.bees and self.lower_bound <= t <= self.upper_bound:
...
class WallAnt(Ant):
    ...
    food_cost=4
    implemented=True
    def __init__(self,health:int=4) -> None:
